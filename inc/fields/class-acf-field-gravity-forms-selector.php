...
class Field extends acf_field
{
    ...
    public function render_field($field)
    {
        ...
        if ($field['allow_null']) {
            $fieldOptions .=
                '<option value="">' .
                __('- Select a form -', 'ACF_GF_FIELD_TEXTDOMAIN') .
                '</option>';
        }
        ...
        foreach ($field['choices'] as $formId => $formTitle) {
            ...
            if (
                (is_array($field['value']) && in_array($formId, $field['value'], false)) ||
            ...
            $fieldOptions .=
                '<option value="' . $formId . '"' . $selected . '>' . $formTitle . '</option>';
        }
    ...
    public function processValue($value, $field)
    {
        ...
        if (class_exists('GFAPI')) {
            ...
            trigger_error('Gravity Forms needs to be activated', E_USER_WARNING);
        }
