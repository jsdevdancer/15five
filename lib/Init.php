...
class Init
{
    ...
    {
        $componentsBasePath = get_template_directory() . '/Components';
        $objectComponentsBasePath = get_template_directory() . '/Components/_Objects';

        Api::registerComponentsFromPath($componentsBasePath);
        Api::registerComponentsFromPath($objectComponentsBasePath);
    ...
    public static function checkRequiredPlugins()
    {
        ...
        if (!$acfActive) {
            self::notifyRequiredPluginIsMissing(
                '<a href="https://www.advancedcustomfields.com/pro/">Advanced Custom Fields PRO</a>'
            );
    ...
    protected static function notifyRequiredPluginIsMissing($pluginName)
    {
        ...
        $message = [
            "${pluginName} plugin not activated. Make sure you activate the plugin on the <a href=\"${pluginUrl}\">plugin page</a>.",
        ];
        $options = [
            'type' => 'error',
